<?php
	$count = count($objects);
	if($count <= 4) $hideButtons = 'if(carousel.options.scroll >= ' . $count . ') {$(".last-objects .jcarousel-prev").hide();$(".last-objects .jcarousel-next").hide();} else {$(".last-objects .jcarousel-prev").show();$(".last-objects .jcarousel-next").show();}';
	else $hideButtons = '';

	$jcarousel = <<<JQUERY
	$('.last-objects.jcarousel').jcarousel({
...
?>
<div class="last-objects jcarousel">
	<ul class="objects">
