<?php
	...
	foreach($objects as $id => $object) {
		...
		if($object->idobjects == $mainObject) {
			$main = 1;
			$center = $object->value('map');
		}
		else $main = 0;
		$markers[] = '[' . $object->value('map') . ', "' . $info . '", ' . $main . ']';
	}
	...
	function initGoogle() {
		...
		$('ul.nav.nav-tabs a[href="#map"]').on('shown', function (e) {
			...
			for(var i = 0; i < markers.length; i++) {
				if(markers[i][3]) 
					...
					var marker = new google.maps.Marker({
						...
						map: map,
						animation: google.maps.Animation.DROP,
						icon: '$baseUrl/images/gps_blue.png',
						content: markers[i][2],
					})
			...
			for(var i = 0; i < markers.length; i++) {
				if(markers[i][3]) 
					...
					var marker = new ymaps.Placemark([markers[i][0], markers[i][1]],{
						...
						iconImageHref: '/images/gps_blue.png', // картинка иконки
						iconImageSize: [22, 22], // размеры картинки
						iconImageOffset: [0, -22] // смещение картинки
					});
